...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    await logActivity('MOVE', {
        ...
        error: failedMoves.length > 0 ? `${failedMoves.length} item gagal dipindahkan.` : undefined
    });

    if (failedMoves.length > 0) {
      return NextResponse.json({ success: false, message: `${fileIds.length - failedMoves.length} item berhasil dipindahkan, ${failedMoves.length} gagal.` }, { status: 207 });
    }

    return NextResponse.json({ success: true, message: `${fileIds.length} item berhasil dipindahkan.` });
