...
export async function GET(request: Request) {
  ...
  try {
    ...
    const logs: string[] = await kv.zrange('zee-index:activity-log', sevenDaysAgo, Date.now(), { byScore: true });
    ...
    if (!adminEmails || adminEmails.length === 0) {
        return NextResponse.json({ message: "Tidak ada admin untuk dikirimi laporan." });
    }
    ...
    return NextResponse.json({ success: true, message: "Laporan mingguan berhasil dikirim." });
  } catch (error: any) {
