...
import { Trash2, RefreshCcw, Loader2, FileX, ArrowDownUp, Check } from "lucide-react";
...
export default function TrashPage() {
  ...
  const [sort, setSort] = useState<{ key: SortKey; order: SortOrder }>({ key: 'modifiedTime', order: 'desc' });
  ...
  useEffect(() => {
    ...
    if (status === "unauthenticated" || (session?.user?.role && session.user.role !== "ADMIN")) {
      addToast({ message: "Akses Ditolak: Halaman ini khusus Admin", type: "error" });
      router.push("/");
    }
  ...
  useEffect(() => {
    if (status === "authenticated" && (session?.user?.role === "ADMIN" || user?.role === "ADMIN")) {
      fetchTrash();
    }
  ...
  const handleBulkAction = async (action: 'restore' | 'delete') => {
      ...
      if (isDelete && !confirm(`Hapus ${selectedFiles.length} item secara permanen? Aksi ini tidak dapat dibatalkan.`)) return;
      ...
      try {
          ...
          if (!response.ok) throw new Error(`Gagal ${isDelete ? 'menghapus' : 'memulihkan'}`);
          addToast({ message: `${selectedFiles.length} item berhasil di${isDelete ? 'hapus' : 'pulihkan'}.`, type: 'success' });
          await fetchTrash();
          setSelectedFiles([]);
      } catch(e) {
          addToast({ message: e instanceof Error ? e.message : 'Terjadi kesalahan', type: 'error' });
      } finally {
  ...
  const renderHeader = () => (
    <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
            <div className="p-3 bg-red-100 text-red-600 rounded-xl"><Trash2 size={24} /></div>
            ...
            {sortedFiles.map((file) => (
                <div key={file.id} className="p-4 flex items-center justify-between hover:bg-muted/50">
                    <div className="flex items-center gap-3 overflow-hidden flex-1">
                        ...
                        />
                        <label htmlFor={`select-${file.id}`} className="truncate cursor-pointer flex-1">
