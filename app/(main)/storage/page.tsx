    ...
    >
      ...
      <div className="mb-8 p-6 bg-card border rounded-lg">
          <div className="flex justify-between items-center mb-2 text-muted-foreground">
            ...
            <span className="font-mono text-lg">{usagePercentage.toFixed(2)}%</span>
          </div>
        ...
        <div className="flex flex-col items-center">
            ...
            <div className="relative w-72 h-72">
                <svg className="w-full h-full transform -rotate-90" viewBox="0 0 240 240">
                    <circle cx="120" cy="120" r={radius} fill="transparent" stroke="hsl(var(--muted))" strokeWidth="25" />
                    {data.breakdown.map((item, index) => {
                        const percentage = totalBreakdownSize > 0 ? (item.size / totalBreakdownSize) * 100 : 0;
                ...
                <AnimatePresence>
                    {hoveredSegment && (
                        ...
                        >
                            ...
                            <p className="text-muted-foreground font-mono">{formatBytes(hoveredSegment.size)}</p>
                            <p className="text-xs text-muted-foreground">({totalBreakdownSize > 0 ? ((hoveredSegment.size / totalBreakdownSize) * 100).toFixed(1) : 0}%)</p>
                        </motion.div>
                    )}
                </AnimatePresence>
            </div>
             <ul className="mt-8 flex flex-wrap justify-center gap-x-6 gap-y-3">
                {data.breakdown.map((item, index) => (
                    <li key={item.type} className="flex items-center text-sm">
                        <span className="h-3 w-3 rounded-full mr-2" style={{ backgroundColor: chartColors[index % chartColors.length] }}></span>
          ...
          <ul className="space-y-2">
             {data.largestFiles.map(file => {
              ...
              <li key={file.id} className="border-b border-border/50 pb-2">
                {shareToken ? (
                    <div className="flex items-center gap-3 p-2 -mx-2 opacity-50 cursor-not-allowed" title="Navigasi dibatasi dalam mode berbagi">
                        {createElement(Icon, { size: 20, className: "text-muted-foreground shrink-0" })}
