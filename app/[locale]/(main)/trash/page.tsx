...
export default function TrashPage() {
  ...
  const fetchTrash = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(t("failedToLoadTrash"));
      const data = await res.json();
      setFiles(data);
    } catch {
      addToast({ message: t("failedToLoadTrash"), type: "error" });
    } finally {
  ...
  const handleBulkAction = async (action: "restore" | "delete") => {
    ...
    if (
      isDelete &&
      !(await confirm(
        t("deletePermanentlyConfirm", { count: selectedFiles.length }),
        { title: t("deletePermanentlyTitle"), variant: "destructive" },
      ))
    )
      return;
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(isDelete ? t("failedToDelete") : t("failedToRestore"));
      addToast({
        message: isDelete
          ? t("itemsDeleted", { count: selectedFiles.length })
          : t("itemsRestored", { count: selectedFiles.length }),
        type: "success",
      });
      await fetchTrash();
      setSelectedFiles([]);
      ...
      addToast({
        message: e instanceof Error ? e.message : t("errorOccurred"),
        type: "error",
      });
    } finally {
