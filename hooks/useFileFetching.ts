...
const fetchFolderPathApi = async (folderId: string, shareToken: string | null) => {
...
export function useFileFetching({
  ...
  const {
    ...
    queryKey: ["files", currentFolderId, shareToken, folderTokens[currentFolderId], refreshKey],
    queryFn: ({ pageParam }) =>
      fetchFilesApi({
