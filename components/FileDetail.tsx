...
const VideoAudioPreview: React.FC<{ src: string, type: 'video' | 'audio', poster?: string, mimeType: string }> = ({ src, type, poster, mimeType }) => {
...
const ImagePreview: React.FC<{ src: string }> = ({ src }) => (
  <img src={src} className="w-full h-full object-contain mx-auto" alt="File preview" />
);
...
const EbookPreview: React.FC<{ src: string }> = ({ src }) => {
  ...
  return (
    <div className="w-full h-full">
      {isLoading && <LoadingPreview />}
      {error && <ErrorPreview message={error} />}
      <div ref={containerRef} className={isLoading || error ? 'hidden' : 'w-full h-full'} />
    </div>
  );
};

const CodePreview: React.FC<{ src: string, fileName: string }> = ({ src, fileName }) => {
...
export default function FileDetail({ file }: { file: DriveFile }) {
  ...
  const [isFetchingEditableContent, setIsFetchingEditableContent] = useState(false);
  ...
  const renderPreview = () => {
    ...
    switch (fileType) {
      case 'video':
        return <VideoAudioPreview src={directLink} type="video" poster={file.thumbnailLink} mimeType={mimeType} />;
      case 'audio':
        return <VideoAudioPreview src={directLink} type="audio" mimeType={mimeType} />;
      case 'image':
        ...
        <div className="lg:col-span-2 flex flex-col flex-1 min-h-0">
            {isEditable && (
             ...
             <div className="mb-2 flex justify-end gap-2">
                    {isEditing && (
                        <button onClick={handleSaveChanges} disabled={isSaving || isFetchingEditableContent} className="px-3 py-1 text-sm bg-primary text-primary-foreground rounded-md hover:bg-primary/90 flex items-center gap-2 disabled:opacity-50">
                            <Save size={16} />
 ...
 ? 'Batal' : 'Edit File'}
           ...
           <div className="w-full flex-1 flex items-start justify-center overflow-hidden bg-muted/20 rounded-lg border">
            {isEditing && isEditable ? (
                (isFetchingEditableContent) ? <LoadingPreview /> :
                <textarea
