...
export default function ImageEditorModal({
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      if (!croppedAreaPixels) throw new Error(t("invalidCropArea"));
      ...
      if (!res.ok) throw new Error(t("failedToSaveImage"));

      addToast({ message: t("imageSaved"), type: "success" });
      triggerRefresh();
      onClose();
    } catch (error: unknown) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black flex flex-col">
      <div className="flex justify-between items-center p-4 bg-black text-white z-10">
        ...
        <h3 className="font-bold">{t("editImage")}</h3>
        <button
          onClick={handleSave}
          disabled={isSaving}
