...
import { X, UploadCloud, FolderPlus, File as FileIcon, Loader2 } from 'lucide-react';
...
export default function UploadModal({ isOpen, onClose }: UploadModalProps) {
  ...
  const handleCreateFolder = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/folder/create', {
        ...
        body: JSON.stringify({ folderName: newFolderName, parentId: currentFolderId }),
      });
  ...
  const updateUploadProgress = (fileName: string, progress: number, status: 'uploading' | 'success' | 'error', error?: string) => {
    setUploads(prev => ({
      ...prev,
      ...
      xhr.onload = () => {
        if (xhr.status >= 200 && xhr.status < 300) {
          ...
          try {
            ...
            updateUploadProgress(file.name, 0, 'error', errorResponse.error || 'Upload gagal');
          } catch {
            updateUploadProgress(file.name, 0, 'error', 'Terjadi kesalahan server');
          }
        }
      };

      xhr.onerror = () => {
        updateUploadProgress(file.name, 0, 'error', 'Kesalahan jaringan saat mengunggah');
      };
            ...
            <div className="space-y-6">
              ...
              >
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  <span className="font-semibold text-primary">Klik untuk memilih file</span> atau seret dan lepas di sini
                </p>
              </div>
              ...
              <form onSubmit={handleCreateFolder} className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button type="submit" disabled={isCreatingFolder || !newFolderName.trim()} className="px-4 py-2 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-colors font-semibold disabled:bg-primary/50 flex items-center gap-2">
                    {isCreatingFolder ? <Loader2 className="animate-spin" size={18}/> : 'Buat'}
                  </button>
                </div>
              </form>
            </div>
                  ...
                  <div key={up.name}>
                    <div className="flex justify-between items-center text-sm mb-1">
                      <p className="truncate font-medium flex items-center gap-2">
                        <FileIcon size={14} /> 
                        <span className="max-w-[200px] truncate">{up.name}</span>
                      </p>
                      <span className={cn(
                        "text-xs font-mono",
                    ...
                    {up.status === 'error' && <p className="text-xs text-red-500 mt-1">{up.error}</p>}
                  </div>
                ))}
              </div>
            )}
