...
export default function ContextMenu({
  ...
  const menuContent = (
    <ul className="py-2 md:py-1 space-y-0.5 md:space-y-0">
      {!isFolder && (
        <MenuItem onClick={onPreview} icon={Eye} label={t("previewOpen")} />
      )}
      ...
      {isArchive && (
        <MenuItem
          ...
          label={
            !isArchivePreviewable
              ? `${t("tooLarge")} (> ${formatBytes(ARCHIVE_PREVIEW_LIMIT_BYTES)})`
              : t("viewArchiveContents")
          }
        />
      )}
      ...
      {isAdmin && (
        <>
          ...
          <MenuItem onClick={onCopy} icon={Copy} label={t("makeACopy")} />
