...
export default function TwoFactorAuthSetup() {
    ...
    const handleDisable = async () => {
        if (!window.confirm("Apakah Anda yakin ingin menonaktifkan Autentikasi Dua Faktor?")) return;
        setIsLoading(true);
        setError('');
        try {
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">Akun Anda dilindungi dengan autentikasi dua faktor.</p>
                        </div>
                    </div>
                    <button onClick={handleDisable} disabled={isLoading} className="px-4 py-2 bg-destructive text-destructive-foreground rounded-md hover:bg-destructive/90 text-sm font-semibold disabled:opacity-50">
                ...
                <div>
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">Tingkatkan keamanan akun Anda dengan mengaktifkan 2FA.</p>
                        </div>
                    </div>
                    ...
                    <AnimatePresence>
                        {!qrCodeDataURL && (
                            ...
                            >
                                {isLoading ? <Loader2 className="animate-spin" /> : 'Aktifkan 2FA'}
                            </motion.button>
                        )}
                    </AnimatePresence>
                        ...
                        {qrCodeDataURL && (
                            ...
                            >
                                <div className="border-t pt-4">
                                    <p className="text-sm text-center mb-4">1. Pindai kode QR ini dengan aplikasi authenticator Anda (e.g., Google Authenticator).</p>
                                    ...
                                    <p className="text-sm text-center my-4">2. Masukkan kode 6 digit yang muncul di aplikasi Anda untuk menyelesaikan proses.</p>
                                    <form onSubmit={handleVerify} className="flex items-center justify-center gap-2">
                                        <input
                                            type="text"
                                        ...
                                        />
                                        <button type="submit" disabled={isLoading || verificationCode.length !== 6} className="px-4 py-2 bg-primary text-primary-foreground rounded-md hover:bg-primary/90 text-sm font-semibold disabled:opacity-50">
