...
import { Loader2, AlertCircle, Upload, Download, Trash2, Pencil, Move, Copy, Share2, UserPlus, UserMinus, LogIn, KeyRound } from 'lucide-react';
...
const LogDetail: FC<{ log: ActivityLog }> = ({ log }) => {
    return (
        <ul className="text-xs text-muted-foreground space-y-1 pl-2">
            {log.itemName && <li><strong>Item:</strong> {log.itemName}</li>}
            {log.userEmail && <li><strong>Oleh:</strong> {log.userEmail}</li>}
            {log.targetUser && <li><strong>Target:</strong> {log.targetUser}</li>}
            {log.status && <li className={log.status === 'failure' ? 'text-red-500' : 'text-green-500'}><strong>Status:</strong> {log.status}</li>}
            {log.error && <li className="text-red-500"><strong>Error:</strong> {log.error}</li>}
        </ul>
    );
};
...
export default function ActivityLogDashboard() {
    ...
    const fetchLogs = useCallback(async (currentOffset: number) => {
        ...
        try {
            const response = await fetch(`/api/admin/activity-log?offset=${currentOffset}`);
    ...
    return (
        <div>
            ...
            <div className="bg-card border rounded-lg">
                {isLoading ? (
                    <div className="flex justify-center items-center h-48"><Loader2 className="animate-spin text-muted-foreground"/></div>
                ) : logs.length === 0 ? (
                                    ...
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div className="flex-1">
                                            <div className="flex justify-between items-center">
                                                <p className="font-semibold">{log.type.replace(/_/g, ' ')}</p>
                ...
                {hasMore && !isLoading && (
                    <div className="p-4 border-t border-border text-center">
                        ...
                        >
                            {isLoadingMore ? <Loader2 className="animate-spin mx-auto" /> : 'Muat Lebih Banyak'}
                        </button>
                    </div>
                )}
            </div>
