...
export async function getAnalyticsData(): Promise<AnalyticsData> {
    ...
    const viewsToday = (await kv.get<number>(`${DAILY_VIEWS_KEY}:${todayKey}`)) || 0;
    const viewsYesterday = (await kv.get<number>(`${DAILY_VIEWS_KEY}:${yesterdayKey}`)) || 0;
    const visitorsToday = (await kv.get<number>(`${DAILY_VISITORS_KEY}:${todayKey}`)) || 0;
    const visitorsYesterday = (await kv.get<number>(`${DAILY_VISITORS_KEY}:${yesterdayKey}`)) || 0;
    ...
    const hourlyViews: { hour: string; views: number; visitors: number }[] = Array(24)
        .fill(0)
        .map((_, i) => ({
    ...
    for (let i = 29; i >= 0; i--) {
        const dayKey = getDayKey(now - i * 86400000);
        const bytes =
            (await kv.get<number>(`${BANDWIDTH_KEY}:${dayKey}`)) || 0;
        const d = new Date(now - i * 86400000);
