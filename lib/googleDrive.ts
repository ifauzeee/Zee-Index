...
export async function getAllDescendantFolders(accessToken: string, rootFolderId: string): Promise<string[]> {
...
                const data: { files?: { id: string }[], nextPageToken?: string | null } = await response.json();
...
            } catch (error) {
                console.error(`Error saat fetching subfolder untuk ${folderId}:`, error);
                
                pageToken = null; 
            }
        } while (pageToken);
...
  const params = new URLSearchParams({
    q: `'${folderId}' in parents and trashed=false`,
    fields: 'nextPageToken, files(id, name, mimeType, size, modifiedTime, createdTime, webViewLink, thumbnailLink, hasThumbnail, parents, trashed)',
    orderBy: 'folder, name',
...
  const params = new URLSearchParams({
    fields: 'id, name, mimeType, size, modifiedTime, createdTime, webViewLink, webContentLink, thumbnailLink, hasThumbnail, parents, owners(displayName, emailAddress), lastModifyingUser(displayName), md5Checksum, imageMediaMetadata(width, height), videoMediaMetadata(width, height, durationMillis), trashed' // <--- 'trashed' DITAMBAHKAN DI SINI
  });
